...
import { TrendingUp, PieChart, Wallet, DollarSign, Activity, Target, Info, AlertTriangle, Layers } from 'lucide-react';

// --- SUB-COMPONENTES VISUAIS (Para organizar o código) ---

const RadarView = ({ assets, alertas }: { assets: any[], alertas: string[] }) => {
  // Pega Top 3 por Score que precisam de aporte
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6 mb-8 animate-in fade-in duration-500">
      <div className="bg-[#1e293b] p-6 rounded-xl border border-green-900/30 shadow-lg">
        ...
        <div className="space-y-3">
          ...
          )) : <p className="text-slate-500">Nenhuma oportunidade clara (Score &gt; 0) no momento.</p>}
        </div>
      </div>
      ...
      <div className="bg-[#1e293b] p-6 rounded-xl border border-red-900/30 shadow-lg">
        ...
        <div className="space-y-3">
          ...
          )) : <p className="text-slate-500">Carteira balanceada. Nenhum risco crítico.</p>}
        </div>
      </div>
    </div>
  );
...
export default function Home() {
  ...
  // ORDEM DAS CATEGORIAS (Radar no final)
  const categories = [
    ...
    { id: 'Radar', icon: <Target size={18} />, label: "Radar" }, // <--- AGORA NO FINAL
  ];
    ...
    <main className="min-h-screen bg-[#0f172a] text-slate-200 font-sans pb-20">
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6">
        ...
        {loading ? (
          ...
          <>
            {/* VIEW RADAR (Separada) */}
            {tab === 'Radar' && <RadarView assets={data?.ativos} alertas={data?.alertas} />}
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-[#0f172a] text-slate-400 uppercase text-xs font-semibold tracking-wider">
                  <tr>
                    ...
                    <th className="p-4 text-right min-w-[140px]">Meta</th>
