...
export const AlertsButton = ({ onFixAsset }: Props) => {
  // Inicializa SEMPRE como array vazio para evitar o erro .map
  const [alerts, setAlerts] = useState<Alert[]>([]);
  const [isOpen, setIsOpen] = useState(false);

  const fetchAlerts = () => {
    fetch('http://localhost:5328/api/alerts')
      .then(res => res.json())
      .then(data => {
        // TRAVA DE SEGURANÇA: Só atualiza se for uma lista real
        if (Array.isArray(data)) {
            setAlerts(data);
        } else {
            console.warn("API de Alertas retornou algo estranho:", data);
            setAlerts([]); 
        }
      })
      .catch(e => {
          console.error(e);
          setAlerts([]); // Em caso de erro, zera a lista
      });
  };
  ...
  return (
    <div className="relative">
      {/* Botão Estilo "Ferramenta" (Igual ao Brain/Refresh) */}
      <button 
        ...
        <>
          ...
          <div className="absolute right-0 mt-2 w-80 bg-[#0f1421] border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-3 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
              ...
              <span className={`text-[10px] px-2 py-0.5 rounded-full ${alerts.length > 0 ? 'bg-red-500/20 text-red-400' : 'bg-green-500/20 text-green-400'}`}>
